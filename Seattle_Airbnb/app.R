  ...
  tabsetPanel(
    ...
    tabPanel("Box Plot Prices",     
             ...
             sidebarLayout(
               ...
               sidebarPanel(
                 h4("Neighborhood Pricing: Box Plot"),
                 p("Select a neighborhood below to see the corresponding prices."),
    ...
    tabPanel('About',
             ...
             column(8, 
                    ...
                    HTML("<p> View the application on <a href = 'https://github.com/jcassell6/Seattle-Airbnb-Shiny-App'> 
                         Jaime's Github </a>"),
                    br(),
                    HTML("<p> View the application on <a href = 'https://github.com/Nikunja1/seattle_Airbnb'> 
                         Nikunja's Github </a>"),
                    hr(),
...
server <- function(input, output) {
  ...
  neighborhood_df <- reactive ({
    AirbnbDF %>% filter(neighbourhood == input$hood)
  })
  
  # Generate a plot of the requested variable against mpg ----
  output$mpgPlot <- renderPlotly({
    df <- neighborhood_df()
    ggplot(data = df, aes(x=as.factor(neighbourhood), y=price)) +
      labs(title = "Prices per Neighborhood",
           y = "Price", x = "Neighborhood") +
      theme(axis.title = element_text(size = 18)) +
      theme(plot.title = element_text(size = 22)) +
      theme(axis.text = element_text(size = 12)) +
